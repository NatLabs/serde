...
module {
    ...
    func infer_candid_types(candid_values : [Candid], renaming_map : TrieMap<Text, Text>) : Result<[CandidTypes], Text> {
    ...
    public func one_shot(candid_values : [Candid], options : ?T.Options) : Result<Blob, Text> {
        ...
        let compound_type_buffer = Buffer.Buffer<Nat8>(8);
        let primitive_type_buffer = Buffer.Buffer<Nat8>(8);
        let value_buffer = Buffer.Buffer<Nat8>(8);
        ...
        unsigned_leb128_64(candid_buffer, counter[C.COUNTER.COMPOUND_TYPE]);
        candid_buffer.append(compound_type_buffer);

        // add primitive type to the buffer
        unsigned_leb128_64(candid_buffer, counter[C.COUNTER.PRIMITIVE_TYPE]);
        candid_buffer.append(primitive_type_buffer);

        // add value to the buffer
        // unsigned_leb128_64(candid_buffer, counter[C.COUNTER.VALUE]);
        candid_buffer.append(value_buffer);

        #ok(
            Blob.fromArray(
                Buffer.toArray(candid_buffer)
            )
        );
    };
    ...
    public func one_shot_encode(
        ...
        func encode_compound_type_only(
            ...
            switch (candid_type) {
                ...
                case (#Record(record_types)) {
                    ...
                    let sorted_record_types = Array.sort<(Text, CandidTypes)>(
                        record_types,
                        func((a, _) : (Text, CandidTypes), (b, _) : (Text, CandidTypes)) : Order {
                            Text.compare(a, b);
                        },
                    );
        ...
        var prev_compound_type_start_index = 0;

        func get_prev_compound_type_start_index(compound_type_buffer : Buffer<Nat8>) : Nat {
            let tmp = prev_compound_type_start_index;
            prev_compound_type_start_index := compound_type_buffer.size();
            tmp;
        };
        ...
        func encode_compound_type(
            ...
            var is_counter_incremented = false;

            switch (candid_type, candid_value) {

                case (#Option(opt_type), #Option(opt_value)) {
                    var curr_start_index = compound_type_buffer.size();
                    ...
                    if (not type_exists) if (not opt_type_is_compound) {
                        compound_type_buffer.add(T.TypeCode.Option);
                    };
                ...
                case (#Record(record_types), #Record(record_entries)) {
                    ...
                    for ((i, ((record_key, value_type), (field_key, field_value))) in Itertools.enumerate(zipped_fields)) {
                        ...
                        if (not type_exists and value_type_is_compound ){
                ...
                case (#Variant(variant_types), #Variant((variant_key, variant_value))) {
                    ...
                    // let sorted_variant_types = Array.sort<(Text, CandidTypes)>(
                    //     variant_types,
                    //     func((a, _) : (Text, CandidTypes), (b, _) : (Text, CandidTypes)) : Order {
                    //         Text.compare(a, b);
                    //     },
                    ...
                    let record_buffer = Buffer.Buffer<Nat8>(8); // will be innefficient
                    let ?variant_index = Array.indexOf<(Text, CandidTypes)>(
                        (variant_key, #Empty),
            ...
            // if (not type_exists and not is_counter_incremented) {
            //     counter[C.COUNTER.COMPOUND_TYPE] += 1;
            // };
    ...
    func internal_to_candid_type(internal_type : InternalCandidTypes, vec_index : ?Nat) : CandidTypes {
        switch (internal_type, vec_index) {
            ...
            case (#Recursive(n, inner_type), _) #Recursive(n, internal_to_candid_type(inner_type, null));
            ...
            for ((index, parent_node) in Itertools.enumerate(above_bottom.vals())) {
                ...
                switch (parent_node.type_) {
                    ...
                    case (#Record(_)) {
                        ...
                        let record_fields : [(Text, CandidTypes)] = Iter.toArray(
                    ...
                    case (#Variant(_)) {
                        ...
                        let variant_types : [(Text, CandidTypes)] = Iter.toArray(
                            Iter.map<CandidTypeNode, (Text, CandidTypes)>(
                                tmp_bottom_iter,
                                func(node : CandidTypeNode) : (Text, CandidTypes) {
                                    get_max_height(node);
